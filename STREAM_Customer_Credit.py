...
with col1 :
    credit_history = st.text_input('Input Credit History ( 0.0 = all paid  ; 1.0 = critical/other existing credit ; 2.0 = delayed previously ; 3.0 = existing paid )')

with col2 :
    credit_amount = st.text_input('Input Credit Amount')

with col1 :
    savings_status = st.text_input('Input Saving Status ( 0.0 = No known savings ; 1.0 = known savings) ')

with col2 :
    employment = st.text_input('Input Employment ( 0.0 = unemployed ; 1.O = working )')

with col1 :
    installment_commitment = st.text_input('Input Installment Comitment')

with col2 :
     personal_status = st.text_input('Input Personal Status ( 1.0 = singgel ; 0.0 = married )')

with col1 :
    other_parties = st.text_input('Input Other Parties ( 0.0 = co applicant ; 2.0 = none ; 1.0 = guarantor')
